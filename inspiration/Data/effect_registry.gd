...
##
## This is an Autoload singleton. Add it to your project as:
##   Node → Script → effect_registry.gd → Name: "EffectRegistry"
##
...
##   3. Done — EffectChainV2 will find it automatically.
##
...
func get_handler(category: int, subtype: int) -> EffectHandler:
	...
	push_error(
		"EffectRegistry: no handler registered for category=%d subtype=%d. " +
		"Add a _register() call in effect_registry.gd._ready()." % [category, subtype]
	)
	return null
...
func _register_all() -> void:
	...
	# ── VISUAL ─────────────────────────────────────────────────────────────────
	_register(T.Category.VISUAL, T.VisualSubtype.SPAWN_HIT_PARTICLES,       SpawnHitParticlesHandler.new())
	_register(T.Category.VISUAL, T.VisualSubtype.SPAWN_EXPLOSION_PARTICLES, SpawnExplosionParticlesHandler.new())
	_register(T.Category.VISUAL, T.VisualSubtype.ANIMATE_DIE_TO_TILE,       AnimateDieToTileHandler.new())
	_register(T.Category.VISUAL, T.VisualSubtype.ATTACK_TWEEN,              AttackTweenHandler.new())
	_register(T.Category.VISUAL, T.VisualSubtype.SHAKE_DICE,                ShakeDiceHandler.new())
	_register(T.Category.VISUAL, T.VisualSubtype.PLAY_SOUND,                PlaySoundHandler.new())
	_register(T.Category.VISUAL, T.VisualSubtype.WAIT,                      WaitHandler.new())
