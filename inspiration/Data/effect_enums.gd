...
##
## NOTE ON INSPECTOR UX:
##   The 'subtype' field on EffectData is exported as a plain int because
##   GDScript cannot export a "conditionally typed" enum based on another
##   export. This means the inspector shows a number, not a dropdown.
##   To fix this, implement an EditorInspectorPlugin that replaces the
##   subtype field with the correct enum dropdown based on the selected
##   category. That's a polish step — the system works fine without it.
## ============================================================
...
enum Category {
	...
	VISUAL,           ## Particles, tweens, sounds, wait
...
## Used when Category == ATTRIBUTE_CHANGE
...
## Used when Category == AMOUNT_MODIFIER
...
## Used when Category == DICE_CONTROL
...
## Used when Category == TILE_CONTROL
...
## Used when Category == SCENARIO_CONTROL
...
## Used when Category == CONDITIONAL
