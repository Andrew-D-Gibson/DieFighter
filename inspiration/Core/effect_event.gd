## EffectEvent
## ============================================================
## Base class for all events that flow through the ScenarioEngine.
##
## Every gameplay action — dealing damage, healing, opening the shop,
## spawning particles, moving a tile — becomes an EffectEvent subclass.
## Subclasses override resolve() to implement their specific behavior.
##
## LIFECYCLE:
##   1. A handler (e.g. DealDamageHandler) creates an event and calls
##      engine.enqueue_event(event).
##   2. ScenarioEngine pops it from the queue.
##   3. Modifier.on_before_event() hooks run — they may adjust amount,
##      targets, or set canceled = true.
##   4. If not canceled, event.resolve(engine) runs.
##   5. Modifier.on_after_event() hooks run — they may enqueue follow-ups.
## ============================================================
