...
This is the same insight as EffectChain itself: one week of handler-writing buys you *permanently free* per-content choreography. A big cannon tile can be authored as `WAIT(100) → SLOW_MO → SPAWN_EXPLOSION → HITSTOP → SCREEN_SHAKE(large) → damage`, all in a `.tres`.
